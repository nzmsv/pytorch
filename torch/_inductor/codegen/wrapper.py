...
class CppWrapperCodeGen(WrapperCodeGen):
    ...
    def codegen_model_kernels(self):
        self.prefix.writeline("struct AOTInductorModelKernels {")
...
class CudaWrapperCodeGen(CppWrapperCodeGen):
    ...
    def generate_load_kernel(self, name, params):
        ...
        if V.graph.aot_mode:
            self.writeline(f"if (this->kernels_->{name} == nullptr) {{")
            self.writeline(
                f"""    this->kernels_->{name} = loadKernel("{cubin_path}", "{mangled_name}", {shared_mem}, this->cubin_dir_);"""
            )
            self.writeline("}")
        else:
    ...
    def generate_kernel_call(
        ...
        kernel_var_name = f"this->kernels_->{name}" if V.graph.aot_mode else name
        self.writeline(
            "launchKernel({}, {}, {}, {}, {}, {}, {}, {});".format(
                kernel_var_name,
                f"{grid_name}.grid_x",
